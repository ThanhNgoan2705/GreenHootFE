...
const coverPhoto = ref(''); // Add this line to define the 'coverPhoto' property
...
if (props.exam) {
  examTitle.value = props.exam.title;
  description.value = props.exam.description;
  coverPhoto.value = props.exam.imageUrl;
}
const checkChangeExamInfor = () => {
   if(examTitle.value !== props.exam.title || description.value !== props.exam.description || coverPhoto.value !== props.exam.imageUrl){
     return true;
   }
}

const handleUpdateExam = (event: Event) => {
  event.preventDefault();
  if(!checkChangeExamInfor()){
    openPopup.value = false;
    return;
  }
  let requestUpdateExam = UpdateExamRequest.create();
  let exam = Exam.create();
  exam.examId = props.exam.examId;
  exam.userId = userId;
  exam.title = examTitle.value;
  exam.description = description.value;
  exam.imageUrl = coverPhoto.value;
  requestUpdateExam.exam = exam;
  console.log(requestUpdateExam);
  let packet = Packet.create();
  packet.data = { oneofKind: 'updateExamRequest', updateExamRequest: requestUpdateExam };
  console.log(packet);
  console.log('sent update exam');
  WS.send(packet);
}
...
<div class="popUp-wrapp" v-if="openPopup">
    ...
    <div class="popup-container p-1">
      <div class="greenhoot-sumary">
        ...
        <div  class="dialog-action ">
          ...
          <button   class="btn-done" @click="handleUpdateExam">Save</button>
        </div>
      </div>
    </div>
  </div>
