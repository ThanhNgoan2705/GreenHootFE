...
const changeBgColorOnInput = (index) => {
  const answerOption = document.querySelectorAll('.answer-option-text');
  const answerContainer = document.querySelectorAll('.answer-option-container');
  const actionButton = document.querySelectorAll('.add-img-btn');
  ...
  fileInput.addEventListener('change', (e) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        const answerContain = document.querySelectorAll('.ans-contain');
        const answerOption = document.querySelectorAll('.answer-option-text');
        const actionButton = document.querySelectorAll('.add-img-btn');
        const radioBtn = document.querySelectorAll('.radio-btn');
        ...
        answerOption.forEach((answer, i) => {
          if (i === index) {
            answerContain[i].style.overflow = 'hidden';
            changeBGColor(index)
            radioBtn[i].style.display = 'block';
            actionButton[i].style.display = 'none';
            removeBtn[i].style.display = 'block';
            answer.innerHTML = '';
            answer.appendChild(imgElement);
          }
        });
      }
    }
...
const markTrueAnswer = (index) => {
  const radioBtn = document.querySelectorAll('.radio-btn');
  const iconTick = document.querySelectorAll('.icon-tick');
...
const removeAnswerImage = (index) => {
  const answerContain = document.querySelectorAll('.answer-option-container');
  const answerOption = document.querySelectorAll('.answer-option-text');
  const actionButton = document.querySelectorAll('.add-img-btn');
  const radioBtn = document.querySelectorAll('.radio-btn');
  const removeBtn = document.querySelectorAll('.remove-btn');
...
<template>
  <div class="answer-options w-full flex  flex-[4_1_0%] flex-wrap h-full content-stretch">
    <div v-for="(item , index) in 4" key="index"
      ...
      <MDBIcon :icon="symbol[index]" size="2xl" class="text-white absolute left-3.5 top-1/2"/>
